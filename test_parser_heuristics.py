...
class TestParserHeuristics(unittest.TestCase):
    ...
    def test_robust_ocr_sentence_fallbacks(self):
        ...
        self.assertEqual(suggestions["father_name"], "")
    ...
    def test_death_case_classification_with_boilerplate(self):
        ...
        ocr_lines = [
            "--- PAGE 1 ---",
            "CLAIM PETITION BEFORE THE TRIBUNAL",
            "Name and description of the injured person / Deceased: Late Ramesh Kumar",
            "In non-fatal accident cases details of injuries: N/A (Fatal accident, deceased died on the spot)",
            "Funeral expenses incurred: Rs. 15,000/-",
            "--- PAGE 2 ---",
            "AWARD COPY",
            "Loss of dependency calculation:",
            "Monthly income: Rs. 10,000/-",
            "Total Compensation: Rs. 12,00,000/-"
        ]
        suggestions = parse_extracted_text(ocr_lines)
        self.assertEqual(suggestions["case_type"], "death")
        self.assertEqual(suggestions["deceased_name"], "Ramesh Kumar")

    def test_death_case_father_name_rejection(self):
        """Verify that any father_name extracted from the claimant section in a death case is rejected when no deceased-specific block matched."""
        ocr_lines = [
            "--- PAGE 1 ---",
            "CLAIM PETITION BEFORE THE TRIBUNAL",
            "Rajesh Kumar S/o Late Shri Om Prakash Goswami",
            "Resident of Bhopal",
            "--- PAGE 2 ---",
            "AWARD COPY",
            "The deceased person Om Prakash Goswami was aged 50 years.",
            "Funeral expenses: Rs. 15,000/-",
            "Total Compensation: Rs. 8,00,000/-"
        ]
        suggestions = parse_extracted_text(ocr_lines)
        self.assertEqual(suggestions["case_type"], "death")
        self.assertEqual(suggestions["deceased_name"], "Om Prakash Goswami")
        self.assertEqual(suggestions["claimant_name"], "Rajesh Kumar")
        self.assertEqual(suggestions["father_name"], "")

    def test_claimant_relationship_extraction_and_formatting(self):
        """Verify that claimant relationship is extracted and formatted correctly for the calculator."""
        ocr_lines = [
            "--- PAGE 1 ---",
            "CLAIM PETITION BEFORE THE TRIBUNAL",
            "Rajesh Kumar S/o Late Shri Om Prakash Goswami",
        ...
        suggestions = parse_extracted_text(ocr_lines)
        self.assertEqual(suggestions["claimant_relationship_to_deceased"], "Son of")
        self.assertEqual(suggestions["claimant_relationship_type"], "Son of")
    ...
    def test_regression_ma_10076_future_type(self):
        """Verify that MA_10076_2025.pdf extracts future_type as 2 (Self-Employed / Fixed Salary), not 1."""
        fixture_path = os.path.join(os.path.dirname(__file__), "ma_10076_ocr_lines.txt")
        self.assertTrue(os.path.exists(fixture_path), f"Fixture not found: {fixture_path}")
        with open(fixture_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
