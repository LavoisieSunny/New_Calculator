...
async def chat_with_pdf(request: PDFChatRequest):
    ...
    try:
        ...
        if request.is_justify:
            ...
            results_award = semantic_search_rag(
                query="compensation awarded amount medical expenses pain suffering transport attender loss of income heads",
                limit=4,
                filename_filter=filename_filter
            )
            results_grounds = semantic_search_rag(
                query="grounds of appeal enhancement disfiguration ear loss marriage prospects disability",
                limit=3,
                filename_filter=filename_filter
            )
